...
HEIGHT = 960
WIDTH = 720
base_dir = "/mnt/sdc/jyw_data/3d_2d/raw_data_640_CFPose"
save_dir = "/mnt/sdc/jyw_data/3d_2d/raw_data_640_CFPose_pred"
...
def main():
    ...
    for i in range(len(files)):
        f = os.path.join(save_dir, files[i])

        if not os.path.exists(f):
            path_img = os.path.join(base_dir,files[i])
            img = loadmat(path_img)['img']
            mask = (img > 4)
            img = equalize_img(img,mask)
            ...
            minu_pred3 = locate_minutiae2(minu_out,ori_out,tau_init=0.2,tau_step=0.2)
