...
from models.utils.layers import Conv3x3,ConvBlock
...
class PixelShuffleLayer1(nn.Module):
    def __init__(self, in_channels, upscale_factor):
        super(PixelShuffleLayer1, self).__init__()
        self.pixel_shuffle = nn.PixelShuffle(upscale_factor)
        # 下述卷积经计算 图像尺寸不变，通道数从第一列变为第二列
        #对要与localMinNet输出图像（通道数为1）进行逐元素相乘，保证通道数和图像尺寸完全一致
        self.conv = nn.Conv2d(in_channels // (upscale_factor ** 2), 1, kernel_size=3, padding=1)
...
class DepthDecoder2(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=[180,1,2,16], use_skips=True):
        super().__init__()
        ...
        self.convs_ori1 = ConvBlock(num_ch_in, num_ch_out)
        num_ch_in = self.num_ch_dec[2]
        if self.use_skips:
            num_ch_in += self.num_ch_enc[1]
        num_ch_out = self.num_ch_dec[2]
        self.convs_ori2 = ConvBlock(num_ch_in, num_ch_out)

        self.convs_ori3 = Conv3x3(self.num_ch_dec[2], self.num_output_channels[0])

        num_ch_in = self.num_ch_enc[-1]
        num_ch_out = self.num_ch_dec[2]
        self.convs_ped1 = ConvBlock(num_ch_in, num_ch_out)
        num_ch_in = self.num_ch_dec[2]
        if self.use_skips:
            num_ch_in += self.num_ch_enc[1]
        num_ch_out = self.num_ch_dec[2]
        self.convs_ped2 = ConvBlock(num_ch_in, num_ch_out)

        self.convs_ped3 = Conv3x3(self.num_ch_dec[2], self.num_output_channels[1])

        num_ch_in = self.num_ch_enc[-1]
        num_ch_out = self.num_ch_dec[2]
        self.convs_grad0 = Conv(num_ch_in, num_ch_in, kSize=3, stride=1, padding=1, bn_act=True)
        self.convs_grad1 = ConvBlock(num_ch_in, num_ch_out)
        num_ch_in = self.num_ch_dec[2]
        if self.use_skips:
            num_ch_in += self.num_ch_enc[1]
        num_ch_out = self.num_ch_dec[2]
        self.convs_grad2 = ConvBlock(num_ch_in, num_ch_out)

        self.convs_grad3 = Conv3x3(self.num_ch_dec[2], self.num_output_channels[2])

        num_ch_in = self.num_ch_enc[-1]
        num_ch_out = self.num_ch_dec[2]
        self.convs_minu0 = Conv(num_ch_in, num_ch_in, kSize=3, stride=1, padding=1, bn_act=True)
        self.convs_minu1 = ConvBlock(num_ch_in, num_ch_out)
        num_ch_in = self.num_ch_dec[2]
        if self.use_skips:
            num_ch_in += self.num_ch_enc[1]
        num_ch_out = self.num_ch_dec[2]
        self.convs_minu2 = ConvBlock(num_ch_in, num_ch_out)

        self.convs_minu3 = Conv3x3(self.num_ch_dec[2], self.num_output_channels[3])
        self.convs_minu4 = PixelShuffleLayer1(num_output_channels[3],upscale_factor=4)
    ...
    def forward(self, input_features,mask1=None,mask2=None,mask3=None):
        self.outputs = {}
        x = input_features[-1]
        ori = self.convs_ori1(x)
        ori = [upsample(ori)]
        if self.use_skips:
            ori += [input_features[1]]
        ori = torch.cat(ori, 1)
        ori = self.convs_ori2(ori)

        ori_out = upsample(self.convs_ori3(ori), mode='bilinear')
        if mask2 is not None:
            ori_out = F.softmax(ori_out, dim=1) * mask2
        else:
            ori_out = F.softmax(ori_out, dim=1)

        ped = self.convs_ped1(x)
        ped = [upsample(ped)]
        if self.use_skips:
            ped += [input_features[1]]
        ped = torch.cat(ped, 1)
        ped = self.convs_ped2(ped)

        ped_out = upsample(self.convs_ped3(ped), mode='bilinear')
        if mask2 is not None:
            ped_out = ped_out *mask2

        minu = self.convs_minu1(x)
        minu = [upsample(minu)]
        if self.use_skips:
            minu += [input_features[1]]
        minu = torch.cat(minu, 1)
        minu = self.convs_minu2(minu)

        minu_out = upsample(self.convs_minu3(minu), mode='bilinear')

        minu_out = self.convs_minu4(minu_out)
        if mask3 is not None:
            minu_out = minu_out *mask3



        grad = torch.cat((ori,ped),dim=1)
        grad = self.convs_grad1(grad)
        grad = [grad]
        if self.use_skips:
            grad += [input_features[1]]
        grad = torch.cat(grad, 1)
        grad = self.convs_grad2(grad)

        grad_out = upsample(self.convs_grad3(grad), mode='bilinear')
        if mask1 is not None:
            grad_out = grad_out *mask1

        return ori_out,ped_out,grad_out,minu_out


class LocalMinNet(nn.Module):
    def __init__(self):
        super(LocalMinNet, self).__init__()
        # 检查没问题
        # Define the dilated convolution layers
        self.dilation1 = nn.Conv2d(in_channels=1, out_channels=2, kernel_size=3, padding=1, dilation=1)
        self.dilation2 = nn.Conv2d(in_channels=2, out_channels=4, kernel_size=3, padding=2, dilation=2)
        self.dilation3 = nn.Conv2d(in_channels=4, out_channels=8, kernel_size=3, padding=4, dilation=4)
        self.dilation4 = nn.Conv2d(in_channels=8, out_channels=16, kernel_size=3, padding=8, dilation=8)

        # Pixel-wise convolution (1x1 convolution)
        self.pixelwise_conv = nn.Conv2d(in_channels=16, out_channels=1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # Apply pixel-wise convolution
        x = self.pixelwise_conv(x)

        return x

class LightMonoAll(nn.Module):
    def __init__(self,height=512,width=512,num_ch_enc=np.array([48, 80, 128]),scales=range(3)):
        super(LightMonoAll, self).__init__()
        self.net1 = LiteMono(height=height,width=width)
        self.net2 = DepthDecoder2(num_ch_enc,scales=scales)
        self.net3 = LocalMinNet()

    def forward(self, x,mask1=None,mask2=None,mask3=None):
        minu = self.net3(x.mean(dim=1).unsqueeze(dim=1))

        x = self.net1(x)
        ori_out,ped_out,grad_out,minu_out = self.net2(x,mask1,mask2,mask3)
        minu_out = minu_out*minu
        return (ori_out,ped_out,grad_out,minu_out,upsample(x[0],4/3*2))
